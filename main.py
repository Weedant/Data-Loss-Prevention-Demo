  ...
  <script>
    ...
    function clearWhitelist() {
      if(confirm('Are you sure you want to remove ALL whitelisted paths? This cannot be undone.')) {
        fetch('/clear_whitelist', {method: 'POST'})
          .then(r => r.json())
          .then(data => {
            alert('Cleared ' + data.removed + ' whitelisted path(s)');
            location.reload();
          })
          .catch(e => alert('Error: ' + e));
      }
...
<body>
  <div class="container">
    ...
    {% if whitelist %}
    <button class="btn btn-delete" onclick="clearWhitelist()" style="margin:10px 0">🗑️ Clear All Whitelist</button>
    {% endif %}
    <ul>
    {% if whitelist %}
      {% for w in whitelist %}
...
def clear_whitelist():
    """Remove all whitelisted paths"""
    with state_lock:
        removed = len(state.get("whitelist", []))
        state["whitelist"] = []
        save_state(state)

    print(f"[INFO] Cleared {removed} whitelisted path(s)")
...
def export_alerts():
    ...
    # Create CSV content as string directly (this was working!)
    csv_lines = []
    ...
    for alert in alerts:
        ...
        file_size = str(alert.get('file_size', 'N/A'))
        rule = str(alert.get('rule', ''))
        time_val = str(alert.get('time', ''))
        status = str(alert.get('status', ''))
        origin = str(alert.get('origin', ''))

        row = f'"{file_path}","{file_size}","{rule}","{time_val}","{status}","{origin}","{original_path}"'
    ...
    if len(csv_content) > 0:
        print(f"[INFO] First 100 chars: {csv_content[:100]}")

    # Create response (this was the working version!)
